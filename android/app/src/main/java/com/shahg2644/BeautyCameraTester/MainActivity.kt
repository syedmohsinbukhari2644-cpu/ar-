...
class MainActivity : ComponentActivity() {
    ...
    private fun createUi() {
        val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setBackgroundColor(0xff090909.toInt()) }; val frame = FrameLayout(this); preview = PreviewView(this).apply { scaleType = PreviewView.ScaleType.FILL_CENTER }; renderer = BeautyRendererView(this); frame.addView(preview, FrameLayout.LayoutParams(-1, -1)); frame.addView(renderer, FrameLayout.LayoutParams(-1, -1)); root.addView(frame, LinearLayout.LayoutParams(-1, 0, 1f))
